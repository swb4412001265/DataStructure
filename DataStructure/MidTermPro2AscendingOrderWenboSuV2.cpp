//Midterm Homework from Wenbo Su
...
bool BagList::putANode (string udata) {
	...
	Node* temp1;
	Node A;
	if (ListPointer == NULL || ListPointer->getNodeData() >= udata)		//if List is NULL or input string is equal or smaller than existing string, 
																		//then put string at the beginning.
	{
	...
	else																//this is the condition that input string is bigger than existing string
	{	
		Node* NEW=ListPointer;
		Node* PREVIUOS=ListPointer;
		while (NEW->getNextPointer()!=NULL && NEW->getNodeData() < udata)	//find out where should insert the new Node.
		{
			PREVIUOS = NEW;	
			NEW=NEW->getNextPointer();
		}
		if (NEW->getNextPointer() == NULL && NEW->getNodeData() < udata)	//if input string is the biggest, then insert string to the end.
		{
			temp1 = A.putNode(NULL,udata);
			NEW->putNextPointer(temp1);
		}
		else																//insert the string in the middle
		{
			temp1 = A.putNode(NEW,udata);
			PREVIUOS->putNextPointer(temp1);
		}
		x = true;
	}	
	return x;
