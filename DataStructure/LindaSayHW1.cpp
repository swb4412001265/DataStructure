...
//
// Revision 7/9/1: Change casts from int to size_t on the four lines commented  
...
int main () {
	...
	if (Arr != NULL) {
		cout << "Value in variable Arr points to the location " 
				<< (size_t) Arr << endl;  // cast to size_t instead of int for 64bit
		...
		cout << "Value in variable Arr points to Heap location " 
								  << (size_t) Arr << endl;  // cast to size_t instead of int for 64bit
		cout << "Location of i is " << (size_t) &i<< endl; // cast to size_t instead of int for 64bit
		cout << "Location of j is " << size_t (&j) << endl; // cast to size_t instead of int for 64bit
		...
		if (temp != NULL) {  // Ask if we if we got the Heap space
			cout << "Reference to new Heap is " << (size_t) temp << endl;
			delete [] temp;
			cout << "New Heap is " << (size_t) temp << endl;
		...
		cout<<"Arr:"<<(size_t)Arr<<endl;
		Arr = temp;  // Assign the reference in temp to reference variable Arr
		cout << "New array is ";
		for (j = 0; j < 10; j++)  // loop to display array elements (again)
			cout << *(Arr+j) << " ";
		cout << endl;
		cout << "Value in variable Arr points to the location " << (size_t) Arr << endl;
		cout << "Location of i is " << size_t (&i) << endl;
		cout << "Location of j is " << size_t (&j) << endl;
		}
		else
