...
public:
  ...
  void postInsertionWrapper(string);
  Node* getRefToRoot ();
...
void BST::postInsertionWrapper(string x)
	{
	// from Kevin Myers
	// need to look for a node which contains string x, then use
	// its reference as the parameter for call to postinsertion balance

	Node* foundNode = NULL;
	foundNode = locateNode(root, x);

	if (foundNode != NULL)
	{
		cout << "Located Node is " << foundNode->getVal() << endl;
		postInsertionBalance(foundNode);
	}
	else
		cout << "Could not find node" << endl;

 }
Node* BST::locateNode(Node* y, string v) {  // Suggested by Dan Glade
	string x;
	x = y->getVal();
	if (v == x)
...
{ string x;
  x = tree -> getVal();
  if ( v < x ) //look to left of curent node
    if( tree -> getLH () !=NULL )
	...
	return tree;		// *** from Paul Pollack 11/13/13 *** prevent 
      ...
      { Node* temp = NULL;
		temp = addNode (temp, v);  // temp is reference to newly added node
		tree ->putLH(temp);
...
void BST::locate (Node* y, string v) {  // Suggested by Dan Glade
  string x;
  x = y -> getVal ();
  if ( v == x )
    { cout << "Value " << v << " is in Node at reference " << (int)y << endl;
...
void BST::postInsertionBalance (Node* n)  //Run after every insertion to    Node* n was param Node* temp in the initial call to this method
	// balance the tree.  Any imbalances can be dealt with by looking at 
	// the parent line to root of the new Node as mentikoned above.
{ Node* p;
...
*/
  ...
  cout << "Add node, show Node, Delete node, Show sorted list, show Tree " << endl;
  cout << "Postinsertionbalance, Help, Exit:" << endl;
	...
	switch (op)
	  ...
	  case 'D': case 'd':
		{if (TREE.getRefToRoot ())
			{ cout << "Which string would you like to delete from the tree?\n";
	  ...
	  case 'P': case 'p':  //From Kevin Myers
		   {if ( TREE.getRefToRoot ())
				{cout << endl << "Enter user string in Node to begin balancing: ";
				 cin >> x;
				 if (!cin.eof())
					 TREE.postInsertionWrapper(x);
				}
		   else
			   cout << "Tree is empty." << endl;
		   break;
...
//			  TREE.postOrderTraverse();
	  ...
	  case 'H': case 'h':
		{ cout << "Enter your operation using the capital letter shown in your selection." << endl; 
		  cout << "Add a node, display a Node, Show sorted list, display Tree," << endl;
		  cout << "Delete a Node, Postinsertion balance, Help, Exit" << endl;
		  break;
		}
	  case 'E': case 'e':
	    { break;
